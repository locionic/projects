...
<AudioPlayer src="/static/audio/aws-lambda-cold-starts.mp3" title="Executive Audio Briefing" />
...
<FAQ
  questions={[
    {
      question: "What causes an AWS Lambda cold start?",
      answer: "A cold start occurs when AWS initializes a new execution environment for your Lambda function. This happens on the initial invocation after deployment, during traffic spikes when new concurrent execution instances are provisioned, or after an idle period (typically 5 to 15 minutes of inactivity)."
    },
    {
      question: "How does AWS Lambda SnapStart eliminate cold start latency?",
      answer: "SnapStart initializes your function ahead of time, captures an encrypted Firecracker microVM snapshot of the memory and disk state, and caches it. On invocation, Lambda resumes from the snapshot rather than running runtime and dependency initialization from scratch, cutting startup latency by up to 90%."
    },
    {
      question: "What is the difference between SnapStart and Provisioned Concurrency?",
      answer: "Provisioned Concurrency keeps a pre-warmed pool of execution environments running 24/7, completely eliminating cold starts at the cost of continuous hourly compute charges. SnapStart resumes cached snapshots on demand with zero idle compute fees, making it significantly more cost-effective for bursty workloads."
    },
    {
      question: "Which programming languages have the fastest cold starts on AWS Lambda?",
      answer: "Compiled languages like Rust and Go, as well as lightweight interpreted runtimes like Node.js and Python, deliver the fastest cold starts (typically 100 to 300 ms). Heavy JVM and .NET runtimes historically experienced 1 to 3 second cold starts, which SnapStart was specifically engineered to mitigate."
    },
    {
      question: "Does increasing memory size reduce cold start duration?",
      answer: "Yes. AWS Lambda allocates CPU power, network bandwidth, and disk I/O proportionally to configured memory. Increasing memory from 512MB to 1792MB grants your function a full dedicated vCPU, significantly accelerating CPU-intensive dependency initialization and JIT compilation during cold starts."
    }
  ]}
/>
