...
<AudioPlayer src="/static/audio/react-server-components-guide.mp3" title="Executive Audio Briefing" />
...
<FAQ
  questions={[
    {
      question: "What is the difference between React Server Components (RSC) and Server-Side Rendering (SSR)?",
      answer: "SSR pre-renders interactive Client Components to initial HTML on the server, but still transfers the entire JavaScript bundle to the browser so React can hydrate state and event listeners. RSC executes exclusively on the server and streams a specialized UI format (the RSC payload); its dependencies and code are never downloaded to the client browser, resulting in zero client bundle size overhead."
    },
    {
      question: "Can a Server Component be passed as a child into a Client Component?",
      answer: "Yes. While a Client Component cannot directly import a Server Component, you can pass a Server Component as a child slot: <ClientWrapper><ServerPostContent /></ClientWrapper>. The server component executes on the backend, and its rendered virtual tree is seamlessly slotted into the interactive client boundary."
    },
    {
      question: "How do Server Actions work alongside React Server Components?",
      answer: "Server Actions are asynchronous functions marked with 'use server' that execute securely on the server. Interactive Client Components can invoke them directly inside form submissions or event handlers to mutate database records, automatically triggering revalidation of surrounding Server Component data trees."
    },
    {
      question: "How do React Server Components improve Interaction to Next Paint (INP) and LCP?",
      answer: "By keeping heavy dependencies (such as date libraries, syntax highlighters, and data transformation packages) entirely on the server, RSC cuts client JavaScript payloads by 30-70%. Less client script execution frees the browser main thread, accelerating Largest Contentful Paint (LCP) and drastically reducing Interaction to Next Paint (INP) latency."
    },
    {
      question: "What data types can be passed across the Server-to-Client component boundary?",
      answer: "Props passed across the boundary must be serializable. Strings, numbers, booleans, plain objects, arrays, and JSX elements are supported. You cannot pass functions, class instances, Symbols, or Date objects directly without prior serialization."
    }
  ]}
/>
