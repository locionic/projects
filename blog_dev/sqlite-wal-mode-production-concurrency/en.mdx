...
<SuccessAlert title="Open-Source Next.js 15 + SQLite SaaS Starter: LiteSaaS">
  Want to deploy this exact high-concurrency SQLite WAL + Litestream architecture without reinventing the wheel? We open-sourced **[LiteSaaS on GitHub](https://github.com/locionic/litesaas)** (with [Live Interactive Demo](https://litesaas-production.up.railway.app)). It comes pre-configured with SQLite WAL mode, Drizzle ORM, self-hosted session authentication, and automated Litestream S3 replication for zero-cost hosting.
</SuccessAlert>
...
### The Hidden Trap: `SQLITE_BUSY_SNAPSHOT` and `BEGIN IMMEDIATE`

Even when `PRAGMA busy_timeout = 5000;` is configured, developers frequently encounter immediate `sqlite3.OperationalError: database is locked` exceptions with **zero wait time**.

This happens due to **deferred transactions** (`BEGIN;` or Python's default transaction model):
1. A transaction starts by executing a `SELECT` statement. SQLite opens a **read transaction** against the current WAL snapshot.
2. In the meantime, another connection commits an `INSERT` or `UPDATE`, advancing the WAL snapshot version.
3. Later in your first transaction, your application attempts an `UPDATE` or `INSERT`. SQLite must now upgrade the connection lock from Reader to Writer.
4. **The conflict:** SQLite cannot allow the write on an outdated snapshot without violating snapshot isolation guarantees.
5. Instead of waiting for `busy_timeout`, SQLite aborts immediately with `SQLITE_BUSY_SNAPSHOT` (extended error code 517).

#### The Solution: Immediate Transactions
Whenever a transaction will perform writes, start it with `BEGIN IMMEDIATE;`:

```python
# Acquire write lock immediately at transaction start
cursor.execute("BEGIN IMMEDIATE;")
cursor.execute("UPDATE accounts SET balance = balance - 100 WHERE id = 1;")
cursor.execute("UPDATE accounts SET balance = balance + 100 WHERE id = 2;")
conn.commit()
```
`BEGIN IMMEDIATE` acquires the `RESERVED` write lock right at the beginning before reading. If another transaction is currently writing, SQLite will honor the full `busy_timeout` and wait for the lock to free up.

---

### Framework Recipes: Production Integration

#### Django / DRF Configuration
In Django, hook the `connection_created` signal to apply production PRAGMAs automatically on every connection:

```python
# In your django app's apps.py or models.py
from django.db.backends.signals import connection_created
from django.dispatch import receiver

@receiver(connection_created)
def configure_sqlite_production(sender, connection, **kwargs):
...
<SuccessAlert title="Ready to Ship with SQLite + Litestream?">
  We implemented this exact production architecture in **[LiteSaaS](https://github.com/locionic/litesaas)** — a 100% free, zero-cost Next.js 15 SaaS starter kit. Experience the [Live Demo](https://litesaas-production.up.railway.app) (instant demo login available) or clone it directly to launch your next SaaS on a $4/mo VPS.
</SuccessAlert>
