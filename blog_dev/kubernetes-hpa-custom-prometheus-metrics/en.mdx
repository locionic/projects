---
title: 'Kubernetes HPA Custom Metrics with Prometheus Adapter'
images: ['/static/images/kubernetes-hpa-custom-prometheus-metrics.jpg']
date: '2026-07-29'
tags: ['kubernetes', 'devops', 'prometheus', 'hpa', 'metrics']
draft: false
summary: 'Learn how to configure Kubernetes Horizontal Pod Autoscaler using custom Prometheus metrics with step-by-step manifests and troubleshooting.'
images:
...
## What Are the Frequently Asked Questions About Custom HPA Metrics?

### Can you scale Kubernetes deployments using custom metrics from external providers like Datadog or CloudWatch?

Yes, you can scale Kubernetes deployments using metrics from external SaaS providers by deploying vendor-supplied metric adapters like the Datadog Cluster Agent or KEDA (Kubernetes Event-driven Autoscaling). These external metric adapters implement the `external.metrics.k8s.io` API interface instead of `custom.metrics.k8s.io`. In your HPA manifest, set `type: External` and specify the metric name defined by your vendor adapter configuration. KEDA is particularly popular because it provides pre-built scalers for over sixty external services including AWS SQS, Azure Event Hubs, Kafka, and PostgreSQL queries.

### What's the difference between Custom Metrics and External Metrics in Kubernetes autoscaling?

The primary difference between Custom Metrics and External Metrics lies in whether the metric is bound to a specific Kubernetes cluster object. Custom Metrics (`custom.metrics.k8s.io`) are strictly attached to Kubernetes resources like Pods or Services running inside your cluster. External Metrics (`external.metrics.k8s.io`) represent global metrics that exist independently of specific cluster objects, such as cloud queue lengths or external database connections.

### How do you verify that Prometheus Adapter rules are properly loaded by the Kubernetes API server?

You verify that Prometheus Adapter rules are properly loaded by querying the extension API using `kubectl get --raw "/apis/custom.metrics.k8s.io/v1beta1"`. This command returns a complete JSON listing of all custom metrics currently exposed by the adapter across your cluster namespaces. If your custom metric name appears in the list, the adapter successfully parsed your configuration rule and validated the series in Prometheus.

### Why does HPA show target average value as unknown after configuring custom metrics?

HPA displays the target average value as unknown when the API server fails to retrieve metric points for target pods from the custom metrics adapter. This condition typically occurs when Prometheus lacks metric data for the targeted workload, pod label matchers are configured incorrectly in adapter rules, or pods were recently deployed. Ensure your application exposes metrics, Prometheus is actively scraping target pods, and label names in your adapter configuration match your Prometheus series labels.

### Should you use rate or instant values when defining PromQL queries in Prometheus Adapter rules?

You should always use rate functions like `rate()` or `irate()` over short time windows when converting Prometheus counter metrics into custom HPA scaling indicators. Counter metrics continuously increase over application lifespan, making them unsuitable for static threshold matching in HPA manifests. Applying `rate()` converts raw counter increments into per-second rates that accurately reflect current traffic intensity.

### How often does the Kubernetes Horizontal Pod Autoscaler re-evaluate custom metric rules?

The Kubernetes Horizontal Pod Autoscaler controller loop evaluates metric rules every fifteen seconds by default. You can customize this evaluation interval by passing the `--horizontal-pod-autoscaler-sync-period` flag to the `kube-controller-manager` component on your cluster control plane nodes. However, decreasing the period below fifteen seconds increases API server load without delivering significant scaling improvements due to application bootstrap latency.

### How do you handle cold-start delays when scaling up pods based on custom metrics?

You handle cold-start delays by combining custom metric HPA rules with appropriate `minReplicas` baselines, pre-warming readiness probes, and optimized container image builds. If your application takes sixty seconds to boot and initialize caches, newly spawned pods won't absorb traffic immediately. Setting aggressive scale-up step policies while ensuring container startup times remain under ten seconds prevents scaling feedback loops during heavy traffic arrivals.
