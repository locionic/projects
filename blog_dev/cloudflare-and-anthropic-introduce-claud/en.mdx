...
<FAQ
  questions={[
    {
      question: "What are Claude Managed Agents on Cloudflare?",
      answer: "Claude Managed Agents combine Anthropic's reasoning models with Cloudflare's serverless edge and microVM sandbox infrastructure. When Claude executes tool calls, bash scripts, or web browsing tasks, the compute runs inside isolated Cloudflare microVM environments with granular network egress controls."
    },
    {
      question: "How does Cloudflare ensure security when running autonomous AI code?",
      answer: "Workloads execute in isolated ephemeral Linux microVMs that share zero kernel state or filesystem access with host infrastructure. Every session is short-lived, external network requests pass through Cloudflare Zero Trust inspection, and detailed audit trails capture every command invoked by the agent."
    },
    {
      question: "How does the built-in browser automation capability work?",
      answer: "Cloudflare provides headless browser instances via Cloudflare Browser Rendering, exposed to Claude through Model Context Protocol (MCP) tools. The agent can navigate complex web pages, render client-side JavaScript, extract structured data, and submit forms without needing self-hosted Puppeteer or Playwright clusters."
    },
    {
      question: "Can I securely connect private databases and internal APIs to the agent?",
      answer: "Yes. By pairing Cloudflare Workers with Cloudflare Tunnels and Hyperdrive, agents can securely query internal private databases and microservices without exposing private IP subnets to the public internet."
    },
    {
      question: "What are the latency and pricing advantages over standard cloud VMs?",
      answer: "Cloudflare's distributed edge places sandbox compute physically closer to external APIs and users, cutting network round-trip latency. Financially, you only pay for active execution CPU milliseconds rather than running dedicated 24/7 cloud virtual machines on standby."
    }
  ]}
/>
